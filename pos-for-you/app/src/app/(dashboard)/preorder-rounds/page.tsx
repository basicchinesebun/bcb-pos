...
import {
  ...
  Layers, TrendingUp, Activity, CalendarClock,
} from 'lucide-react'
...
// ── Header Stats Row ──────────────────────────────────────────────────────────
function StatsRow({ rounds }: { rounds: PreorderRound[] }) {
  const totalRounds    = rounds.length
  const activeRounds   = rounds.filter(r => r.status === 'open').length
  const upcomingRounds = rounds.filter(r => r.status === 'upcoming').length
  const totalReserved  = rounds.reduce((sum, r) => sum + r.items.reduce((s, i) => s + i.reserved, 0), 0)
  ...
  return (
    <div className="grid grid-cols-4 gap-3 px-5 py-3 shrink-0">
      {stats.map(({ label, value, icon: Icon, accent, bg }) => (
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-lg font-bold leading-tight tabular-nums" style={{ color: accent }}>{value}</p>
            <p className="text-xs truncate" style={{ color: 'var(--text-muted-col)' }}>{label}</p>
          </div>
        </div>
      ))}
    </div>
...
function QuotaBar({ item }: { item: RoundItem }) {
  ...
  const pctLabel =
    pct > 80  ? { text: `${pct}%`, cls: 'text-red-400' } :
    pct >= 50 ? { text: `${pct}%`, cls: 'text-yellow-400' } :
                { text: `${pct}%`, cls: 'text-green-400' }
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 flex items-center gap-2 min-w-0">
        {/* Segmented bar with glow on high fill */}
        <div
        ...
        >
          <div
            ...
            style={{
              width: `${pct}%`,
              background: barColor,
              boxShadow: pct > 80 ? `0 0 6px ${barColor}80` : 'none',
            }}
          />
        </div>
        <span className={`text-xs w-8 text-right tabular-nums font-semibold ${pctLabel.cls}`}>
          {pctLabel.text}
        </span>
        <span className="text-xs w-12 text-right tabular-nums" style={{ color: remaining === 0 ? '#dc2626' : 'var(--text-muted-col)' }}>
...
function RoundCard({
  ...
  const totalCapacity = round.items.reduce((s, i) => s + i.maxQty, 0)
  const overallPct    = totalCapacity > 0 ? Math.round((totalReserved / totalCapacity) * 100) : 0
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-4 shrink-0">
          {/* Overall fill mini-ring */}
          <div className="text-right hidden sm:block">
            <p
              className="text-lg font-bold leading-tight tabular-nums"
              style={{
                color: overallPct > 80 ? '#dc2626' : overallPct >= 50 ? '#eab308' : '#22c55e',
              }}
            >
              {overallPct}%
            </p>
            <p className="text-xs" style={{ color: 'var(--text-muted-col)' }}>filled</p>
          </div>
        ...
        <div className="border-t" style={{ borderColor: 'var(--border-col)' }}>
          ...
          <div className="px-5 pt-3 pb-1 flex items-center gap-4">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs" style={{ color: 'var(--text-muted-col)' }}>{'<50%'}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full bg-yellow-500" />
              <span className="text-xs" style={{ color: 'var(--text-muted-col)' }}>50–80%</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full bg-red-500" />
              <span className="text-xs" style={{ color: 'var(--text-muted-col)' }}>{'>80%'}</span>
            </div>
          </div>
          ...
          <div className="px-5 pt-1 pb-2">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1 text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted-col)' }}>Fill</div>
            </div>
          ...
          {/* Action buttons */}
          <div className="px-5 py-3 flex items-center gap-2 border-t" style={{ borderColor: 'var(--border-col)' }}>
            <button
              onClick={e => { e.stopPropagation(); onToggle(round.id) }}
            ...
            <button
              onClick={e => { e.stopPropagation(); onViewOrders(round.id) }}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all active:scale-95"
...
function OrdersPanel({
  ...
  return (
    <div
      ...
      style={{ width: 540, background: 'var(--card-bg)', borderColor: 'var(--border-col)' }}
    >
      {/* Header */}
      ...
      {orders.length > 0 && (
        <div
          className="grid grid-cols-4 gap-0 border-b shrink-0"
          style={{ borderColor: 'var(--border-col)' }}
        >
          {[
            { label: 'Total',     value: orders.length,        color: 'var(--text-col)' },
            { label: 'Confirmed', value: confirmed,            color: '#22c55e' },
            { label: 'Pending',   value: pending,              color: '#eab308' },
            { label: 'Value',     value: fmt(totalValue),      color: '#f97316' },
          ].map(({ label, value, color }) => (
            <div key={label} className="flex flex-col items-center py-2.5 px-2">
              <span className="text-sm font-bold tabular-nums" style={{ color }}>{value}</span>
              <span className="text-[10px] mt-0.5" style={{ color: 'var(--text-muted-col)' }}>{label}</span>
            </div>
          ))}
        </div>
      )}
            ...
            {orders.map(order => (
              <div key={order.id} className="px-5 py-4">
                ...
                <div className="space-y-0.5 mb-2 pl-1">
                  ...
                  <div
                    className="flex items-center justify-between text-sm font-bold pt-1.5 border-t mt-1"
                    style={{ borderColor: 'var(--border-col)' }}
                  >
      ...
      {/* Panel footer: rejected count warning */}
      {rejected > 0 && (
        <div
          className="flex items-center gap-2 px-5 py-3 border-t text-xs"
          style={{ borderColor: 'var(--border-col)', color: '#f87171' }}
        >
          <AlertCircle size={13} />
          {rejected} order{rejected > 1 ? 's' : ''} rejected in this round
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--main-bg)' }}>
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          ...
          <div>
            <h1 className="font-bold text-base leading-tight" style={{ color: 'var(--text-col)' }}>
              ຮອບພຣີອໍເດີ
              <span className="font-normal text-sm ml-2" style={{ color: 'var(--text-muted-col)' }}>Pre-order Rounds</span>
            </h1>
          </div>
        ...
        {/* Stats row */}
        <StatsRow rounds={rounds} />

        {/* Tabs */}
        <div className="flex items-center gap-1 px-5 shrink-0 border-b" style={{ borderColor: 'var(--border-col)' }}>
          {(['active', 'past'] as const).map(tab => (
            ...
            >
              <span className="flex items-center gap-1.5">
                {tab === 'active' ? 'Active Rounds' : 'Past Rounds'}
                <span
                  className="text-xs px-1.5 py-0.5 rounded-full font-bold"
                  style={{
                    background: activeTab === tab ? '#f97316' : 'var(--active-bg)',
                    color:      activeTab === tab ? 'white'   : 'var(--text-muted-col)',
                  }}
                >
                  {tab === 'active' ? activeRounds.length : pastRounds.length}
                </span>
              </span>
            </button>
          ))}
